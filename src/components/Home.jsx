  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      for (const event of movieTitles) {
        try {
          const response = await fetch(`https://www.omdbapi.com/?apikey=${apiKey}&t=${event.title}`);
          if (response.ok) {
            const movie = await response.json();
            movieData.push(movie);
          } else {
            console.error(`Failed to fetch data for ${event.title}. Status code: ${response.status}`);
          }
        } catch (error) {
